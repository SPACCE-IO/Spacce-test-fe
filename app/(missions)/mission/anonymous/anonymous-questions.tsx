...
import { useLazyGetUsersQuery } from "@/store/services/employeeManagement";
...
const AnonymousQuestions = ({
  ...
  const [logResponse] = useLogResponseMutation();
    const [getUsers, {data:users,isLoading,isSuccess,isError}] = useLazyGetUsersQuery();
    ...
    const [userDropdownOpen, setUserDropdownOpen] = useState<{ [key: number]: boolean }>({});
...
useEffect(() => {
  if (users && Array.isArray(users)) {
    const newFilteredUsers: { [key: number]: any[] } = {};
    
    questions.forEach(question => {
      if (question.questionTypeId === 7) {
        const query = searchQuery[question.id] || '';
        if (query.trim() === '') {
          newFilteredUsers[question.id] = users.slice(0, 10); // Show first 10 users by default
        } else {
          newFilteredUsers[question.id] = users.filter((user: any) =>
...
case 5: // Matching/Sorting
  ...
  return (
    <div className="space-y-2 relative">
        ...
        <input
          ...
          onChange={(e) => {
            if (!isAnsweredCorrectly && !isSubmitting) {
              ...
              setSearchQuery(prev => ({ ...prev, [questionId]: value }));
              
              // Open dropdown when user starts typing
              if (value.length > 0) {
                setUserDropdownOpen(prev => ({ ...prev, [questionId]: true }));
              }
            }
          }}
          onFocus={() => {
      ...
      {/* Dropdown */}
      {userDropdownOpen[questionId] && !isAnsweredCorrectly && !isSubmitting && (
        <div className="absolute z-50 w-full bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto">
          ...
          ) : filteredUsers[questionId] && filteredUsers[questionId].length > 0 ? (
            <>
              {filteredUsers[questionId].map((user: any) => {
                const isSelected = answers[questionId] === user.userId.toString();
                return (
                  <div
                    key={user.userId}
                    onClick={() => {
                      // Set the selected user
                      handleAnswerChange(questionId, user.userId.toString());
                      
                      // Update search query to show selected user's name
                      setSearchQuery(prev => ({ 
                        ...prev, 
                        [questionId]: `${user.firstName} ${user.lastName}` 
                      }));
                      
                      // Close dropdown
                      setUserDropdownOpen(prev => ({ ...prev, [questionId]: false }));
                    }}
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-shrink-0">
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                          user.userType === 'R' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                        }`}>
                          {user.userType === 'R' ? 'Regular' : user.userType}
                        </span>
                      </div>
              ...
              {users && filteredUsers[questionId].length < users.length && (
                <div className="p-2 text-center text-xs text-gray-500 bg-gray-50">
                  Showing {filteredUsers[questionId].length} of {users.length} users
                </div>
              )}
            </>
          ) : (
            <div className="p-4 text-center text-gray-500">
              {searchQuery[questionId] ? 'No users found matching your search.' : 'No users available.'}
            </div>
          )}
        </div>
      )}
      ...
      {answers[questionId] && (
        <div className="mt-2">
          {(() => {
            const selectedUserId = parseInt(answers[questionId]);
            const selectedUser = users?.find((user: any) => user.userId === selectedUserId);
            ...
            if (selectedUser) {
              ...
              return (
                <div className="text-sm text-gray-500">
                  Selected user ID: {answers[questionId]} (User details not found)
                </div>
              );
            }
          })()}
