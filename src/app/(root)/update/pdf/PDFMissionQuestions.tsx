...
const PDFMissionQuestions = ({
  questions,
  onSubmit,
  missionType = "VIDEO_MISSION",
  fileUrl = "",
}: MissionVideoQuestionsProps) => {
  console.log("MissionVideoQuestions Props:", {
    questions,
    onSubmit,
    missionType,
    fileUrl,
  ...
  const handleDrop = (
    ...
    if (draggedItem) {
      ...
      const currentQuestion = displayQuestions.find(
        (_, index) => index + 1 === questionId
      );
      if (
        currentQuestion?.questionTypeId === "W" &&
        "matchingItems" in currentQuestion &&
        currentQuestion.matchingItems
      ) {
  ...
  const getOrderedImages = (question: Question, questionId: number) => {
    ...
    if (savedOrder) {
      const orderIds = savedOrder.split(",");
      return orderIds
        .map((id) => question.matchingItems?.find((item) => item.id === id))
                ...
                <div className="space-y-3">
                  {getOrderedImages(question, questionId).map((item, index) => (
                    <div key={`ordered-${item.id}-${index}`}>
                      ...
                      {index <
                        getOrderedImages(question, questionId).length - 1 && (
                        <div
                          onDragOver={handleDragOver}
                  ...
                  <div
                    ...
                    onDrop={(e) =>
                      handleDrop(
                        e,
                        getOrderedImages(question, questionId).length,
                        questionId
                      )
                    }
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap gap-2">
              {Array.from(
                {
                  length:
                    (question.maxRating || 10) - (question.minRating || 1) + 1,
                },
                (_, index) => {
...
export default PDFMissionQuestions;
