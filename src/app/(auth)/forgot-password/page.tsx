...
export default function ForgotPasswordPage() {
  ...
  useEffect(() => {
    if (forgotPasswordProps.isError) {
      setError(
        "Failed to send reset code. Please check your email and organization code."
      );
      setSuccessMessage("");
    }
  ...
  useEffect(() => {
    if (verifyPasscodeProps.isSuccess && verifyPasscodeProps.data) {
      ...
      if (token) {
        ...
        router.push(
          `/reset-password?e=${encodeURIComponent(
            email
          )}&o=${encodeURIComponent(orgCode)}&t=${token}`
        );
      }
    }
  }, [
    verifyPasscodeProps.isSuccess,
    verifyPasscodeProps.data,
    email,
    orgCode,
    dispatch,
    router,
              ...
              <form onSubmit={handleSendPasscode} className="space-y-6">
                <div className="text-center space-y-2">
                  ...
                  <p className="text-gray-400 text-base">
                    Enter your email and organization code to receive a reset
                    passcode.
                  </p>
                </div>
              ...
              <form onSubmit={handleVerifyPasscode} className="space-y-6">
                ...
                <div className="bg-gray-800/30 rounded-lg p-3">
                  <p className="text-gray-300 text-sm">
                    <span className="text-gray-400">Passcode sent to:</span>{" "}
                    {email}
                  </p>
                </div>
