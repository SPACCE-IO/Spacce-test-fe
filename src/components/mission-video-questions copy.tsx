...
const MissionVideoQuestions = ({
    ...
    questions.length > 0
      ...
      : [
          ...
          },
          {
            ...
            question:
              "Arrange these steps in the correct order for making coffee:",
  ...
  const renderQuestionInput = (question: Question, questionIndex: number) => {
    ...
    switch (question.questionTypeId) {
      ...
      case "P": // Image Choice
        const selectedImages = answers[questionId]
          ? answers[questionId].split(",")
          : [];
        return (
            ...
            {question.imageOptions?.map((imageOption) => (
              <div
                ...
                className={`border-2 rounded-lg p-2 cursor-pointer transition-colors ${
                  selectedImages.includes(imageOption.id)
                    ? "border-blue-500 bg-blue-50"
                    : "border-gray-300"
                }`}
                onClick={() =>
                  handleMultipleChoiceChange(questionId, imageOption.id, false)
                }
              >
      ...
      case "W": // Sorting
        const currentItems =
          sortedItems[questionId] || question.sortingItems || [];
        return (
          <div className="space-y-2">
                  ...
                  <button
                    onClick={() =>
                      moveItem(
                        questionId,
                        itemIndex,
                        Math.min(currentItems.length - 1, itemIndex + 1)
                      )
                    }
        ...
        <div className="flex h-full">
          ...
          <div className="w-80 bg-[#A9A9A9] p-8 overflow-y-auto">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Follow the link to view the relevant leadership blueprints and
              thereafter match the pictures and names
            </p>
          </div>
